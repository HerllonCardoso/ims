...
beforeEach(() => { vi.mocked(api.remove).mockReset(); });
...
describe('DeleteConfirmDialog', () => {
  it('calls api.remove with recursive=false by default', async () => {
    ...
    render(<DeleteConfirmDialog path="/a.txt" isDirectory={false} onClose={vi.fn()} onDeleted={onDeleted} />);
  ...
  it('does not show the recursive checkbox for files', () => {
    render(<DeleteConfirmDialog path="/a.txt" isDirectory={false} onClose={vi.fn()} onDeleted={vi.fn()} />);
    expect(screen.queryByRole('checkbox')).not.toBeInTheDocument();
  });

  it('shows the recursive checkbox for directories', () => {
    render(<DeleteConfirmDialog path="/d" isDirectory={true} onClose={vi.fn()} onDeleted={vi.fn()} />);
